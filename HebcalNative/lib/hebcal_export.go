...
// safeCall wraps an exported entry point and converts any panic from the
// underlying hebcal/hdate libraries into a JSON `{"error": "..."}` response.
// Without this, a Go panic crossing the CGO boundary tears down the entire
// .NET process (exit code 2) and bypasses managed try/catch handlers.
func safeCall(fn func() *C.char) (result *C.char) {
	defer func() {
		if r := recover(); r != nil {
			result = toCString(map[string]interface{}{
